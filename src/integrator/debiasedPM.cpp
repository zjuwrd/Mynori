...
class DebiasedPM: public Integrator
{
    ...
    private:
                    ...
                    for(int y=0;y<size.y();++y)
                    {
                        for(int x=0;x<size.x();++x)
                        {
                            ...
                            for(uint32_t samplecnt=0;samplecnt< sampler->getSampleCount();++samplecnt)
                            {
                                ...
                                Kblock.put(pixelSample, KimageVal);
        ...
        EstimateRes estimate(const uint32_t j, const float pmf, const Scene *scene, Sampler *sampler, const Ray3f &ray_) const {
            ...
            EstimateRes res;
            const float rad[3]= {init_Rad * std::pow(float(j),(alpha-1.f)/2.f),
                                init_Rad * std::pow(float(j+1),(alpha-1.f)/2.f),
                                init_Rad * std::pow(float(init_k),(alpha-1.f)/2.f)};
        ...
        float init_Rad =  0.2f;
        float m_radius = 0.2f;
        static constexpr uint32_t PhotonsUnit = 100000;
